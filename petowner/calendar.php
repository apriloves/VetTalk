...
<?php 
  ...
  function build_calendar($month, $year)
  {
    ...
    /*$query = $data->prepare('SELECT * FROM bookings WHERE MONTH(date)=? AND YEAR(date)=? ');
    ...
    while($currentDay <= $numberDays)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $calendar.="<td class='$today'><h5>$currentDay</h5> <a href='book.php?date=".$date."'class='btn btn-success btn xs'> Book 
